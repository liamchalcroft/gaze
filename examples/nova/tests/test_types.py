...
class TestHarnessTypes:
    """Harness-specific type helpers."""

    def test_generation_log_token_total(self):
        """GenerationLog tokens property sums prompt and completion tokens."""
        from radiant_harness.models import GenerationLog

        log = GenerationLog(prompt_tokens=100, completion_tokens=40, finish_reason="stop")
        assert log.tokens == 140

    def test_tool_result_formatted_results(self):
        """formatted_results returns string value when present."""
        from radiant_harness.types import ToolResult

        result = ToolResult(
            tool_name="search_web",
            description="found",
            metadata={"formatted_results": "formatted"},
        )
        assert result.formatted_results == "formatted"

        result_no_format = ToolResult(tool_name="zoom", description="ok", metadata={})
        assert result_no_format.formatted_results is None
