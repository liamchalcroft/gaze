...
``RadiantHarnessAdapter``, ``BaseMultiTurnEnv``, and ``VerifiableProcessorMixin``
require the ``verifiers`` and ``datasets`` packages at runtime. They are lazily
imported so that ``from radiant_harness.verifiers import ExactMatchReward`` (and
...
def __getattr__(name: str):
    ...
    ``RadiantHarnessAdapter``, ``BaseMultiTurnEnv``, and
    ...
    """
    if name == "RadiantHarnessAdapter":
        from .adapter import RadiantHarnessAdapter

        return RadiantHarnessAdapter
