...
def _http_get(url: str, *, timeout: int = 15, allow_redirects: bool = True, head_only: bool = False, user_agent: str | None = None):
    parsed = urlparse(url)
...
def _download_url(url: str, raw_dir: str, request_delay: float, verbose: bool = False, robots_mode: str = "strict") -> Tuple[str, str, bool]:
    """
    ...
    if _should_skip_url(url):
        if verbose:
            print(f"  ⛔ Skipping problematic URL pattern: {url}")
        return url, "", False
    ...
    if url.lower().endswith('.pdf') and _should_skip_pdf(url):
        if verbose:
            print(f"  ⛔ Skipping large PDF (>25MB): {url}")
        return url, "", False
    ...
    if resp is None:
        if verbose:
            print(f"  ✗ Failed download (connection/timeout): {url}")
        return url, "", False
    if not resp.ok:
        if verbose:
            print(f"  ✗ Failed download (HTTP {resp.status_code}): {url}")
        return url, "", False

    # If conditional GET returns 304, reuse existing file
    if resp.status_code == 304 and out_file.exists():
        if verbose:
            print(f"  ✓ Not modified (ETag): {url}")
        return url, str(out_file), True
    ...
    if ext == '.html' and _is_duplicate_html(resp.text):
        if verbose:
            print(f"  ✗ Duplicate HTML (SimHash): {url}")
        return url, "", False

    if verbose:
        kb = len(resp.content) // 1024
        print(f"  ↳ downloaded → {out_file.name} ({kb} KB)")
...
def _process_file(
    ...
    try:
        if ext == '.pdf':
            try:
                ...
                if verbose:
                    print(f"  ⚠️ PDF parsing failed, trying as HTML: {url} - {e}")
                ...
                try:
                    ...
                    if soup is not None:
                        ...
                        for a in original_soup.find_all('a', href=True):
                            ...
                            if (nxt.startswith(base_domain) and 
                                nxt.startswith('http') and 
                                'mailto:' not in nxt and 
                                'javascript:' not in nxt and
                                not _should_skip_url(nxt)):
                    ...
                    if verbose:
                        print(f"  ✗ Text extraction completely failed: {url} - {e2}")
                    return [], []
        else:
            ...
            try:
                ...
                if not text or len(text.strip()) < 50:
                    if verbose:
                        print(f"  ✗ Insufficient content after extraction: {url}")
                    return [], []
                ...
                if verbose:
                    print(f"  ✗ Text extraction completely failed: {url} - {e}")
                return [], []
            
    except Exception as e:
        if verbose:
            print(f"  ✗ Text extraction failed: {url} - {e}")
        return [], []
    ...
    if verbose and new_urls:
        print(f"    ↪ discovered {len(new_urls)} new URLs from {url}")
    ...
    try:
        if len(text) > 100:
            ...
            if lang != 'en':
                if verbose:
                    print(f"  ✗ skipped (non-English lang={lang})")
                return [], new_urls
    except Exception:
        pass
...
def ingest_nice(
    ...
    *,
    ...
    # Collect seed URLs
    source_keys = [
        'nice_urls', 'radiopaedia_urls', 'mriquestions_urls', 
        'aan_urls', 'acr_urls', 'rcr_urls',
    ]
        ...
        try:
            ...
            for loc in tree.findall(".//{*}loc"):
                if loc.text:
                    ...
                    if (base_url.rstrip('/') in txt and 
                        not _should_skip_url(txt) and
                        not re.search(r'\?(page|search|filter)=', txt)):
                        locs.append(txt)
            ...
            if verbose:
                print(f"  ⚠️ Sitemap expansion failed for {base_url}: {e}")
            expanded_urls.append(base_url)
    ...
    if not urls:
        raise ValueError(f"No URLs found in config")
    ...
    for current_depth in range(crawl_depth + 1):
        ...
        if num_workers > 1:
            with ThreadPoolExecutor(max_workers=num_workers) as executor:
                download_futures = {
                    executor.submit(_download_url, url, raw_dir, request_delay, verbose, robots_mode): url 
                    for url, _ in current_batch if url not in visited
                }
            ...
            for url, _ in current_batch:
                if url in visited:
                    continue
                visited.add(url)
                url, filepath, success = _download_url(url, raw_dir, request_delay, verbose, robots_mode)
                if success:
                    download_results.append((url, filepath))
        ...
        if num_workers > 1:
            with ThreadPoolExecutor(max_workers=num_workers) as executor:
                process_futures = {
                    executor.submit(_process_file, url, filepath, include_keywords, exclude_patterns, settings, verbose): url
            ...
            # Sequential fallback
            for url, filepath in download_results:
                docs, new_urls = _process_file(url, filepath, include_keywords, exclude_patterns, settings, verbose)
