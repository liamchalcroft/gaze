...
class CrossEncoderReranker:
    ...
    def rerank(
        self, query: str, documents: list[tuple[str, float]], top_k: int = 5
...
class MedicalConceptExtractor:
    ...
    def extract_concepts(self, text: str) -> dict[str, list[str]]:
        ...
        extracted = {"anatomical": [], "pathological": [], "descriptive": []}
...
class RetrievalPipeline:
    ...
    def retrieve(
        ...
        dense_results = list(unique_results.items())
    ...
    def _determine_reasoning_type(self, text: str, concepts: dict[str, list[str]]) -> str:
        ...
        if any(
            "symmetry" in concept or "bilateral" in concept
            for concept in concepts.get("anatomical", [])
        ):
            return "symmetry_analysis"
        elif any(
            "lesion" in concept or "mass" in concept for concept in concepts.get("pathological", [])
        ):
            return "lesion_detection"
        elif "midline" in text_lower or "deviation" in text_lower:
            return "midline_analysis"
        elif any(
            "hemorrhage" in concept or "stroke" in concept
            for concept in concepts.get("pathological", [])
        ):
    ...
    def _calculate_confidence(
        self, score: float, concepts: dict[str, list[str]], query: str
    ) -> float:
