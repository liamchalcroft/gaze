...
def evaluate_caption(
    ...
    clinical_terms = {"lesion","tumor","hemorrhage","infarct","cyst","atrophy","metastasis","edema"}
    mod_f1s = []
    clin_f1s = []
    abnormal_preds = 0
    for p, r in zip(preds, refs):
        ...
        f1_clin = (2*prec_clin*rec_clin/(prec_clin+rec_clin)) if (prec_clin+rec_clin)>0 else 0.0
    ...
    binary_f1 = float((2*(abnormal_preds/len(preds))*(1.0)/( (abnormal_preds/len(preds))+1.0) ) if preds else 0.0)
    return {
