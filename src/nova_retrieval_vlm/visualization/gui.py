                    ...
                    try:
                        ...
                        if show_calibration and "confidence" in res:
                            ...
                            with col1:
                                ...
                                if "confidence_level" in res:
                                    st.metric("Confidence Level", res["confidence_level"].title())

                            with col2:
                                tokens = res.get("generation_log", {}).get("tokens", 0)
                                cost = res.get("generation_log", {}).get("cost", 0)
                        ...
                        if show_reasoning:
                            with st.expander("🧠 Detailed Reasoning", expanded=False):
                                reasoning = res.get("reasoning", "No detailed reasoning available")
                                st.write(reasoning)

                                # Show confidence reasoning if available
                                if (
                                    "final_response" in res
                                    and "reliability_flag" in res["final_response"]
                                ):
                                    reliability = res["final_response"]["reliability_flag"]
