...
# Backstop against decompression-bomb DoS on untrusted images. The primary guard
# is the per-load ``max_image_dimension`` check in base.py (applied before the
# pixel buffer is allocated); this caps any PIL decode path that lacks one.
# 300 MP comfortably exceeds the default 16384px limit (268 MP) while rejecting
# pathological inputs that expand to billions of pixels.
