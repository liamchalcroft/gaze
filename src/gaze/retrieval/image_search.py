...
def _build_keyword_patterns(
    ...
    prefix_keywords: frozenset[str] = frozenset(),
) -> tuple[tuple[re.Pattern[str], str], ...]:
    """Build word-boundary regex patterns sorted by keyword length (longest first).

    Keywords in *prefix_keywords* use a leading ``\\b`` only, so they match
    derived forms (e.g. "mammograph" matches "mammography").  All other
    keywords use boundaries on both sides to prevent false positives from
    substring matches (e.g. "pet" in "competent", "ct" in "infected").
    """
    ...
    for kw, val in keywords.items():
        ...
        items.append((len(kw), pat, val))
    items.sort(key=lambda x: x[0], reverse=True)
    return tuple((pat, val) for _, pat, val in items)


# Word-boundary regex patterns for modality detection, sorted longest-first
# so more specific keywords take priority (e.g. "ct scan" before "ct").
# Compiled once at import time.
_MODALITY_PATTERNS: tuple[tuple[re.Pattern[str], str], ...] = _build_keyword_patterns(
    {
        "mri": "MRI",
    ...
    prefix_keywords=frozenset({"mammograph", "radiograph"}),
)

_BODY_PART_PATTERNS: tuple[tuple[re.Pattern[str], str], ...] = _build_keyword_patterns(
    {
        "brain": "brain",
        "cerebral": "brain",
        "head": "head",
    ...
    prefix_keywords=frozenset({"vertebr"}),
)
...
class OpenISearchEngine(BaseSearchEngine[ImageSearchResult, ImageSearchError]):
    ...
    def _parse_results(self, data: dict[str, Any]) -> list[ImageSearchResult]:
        ...
        if not isinstance(items, list):
            logger.warning(
                f"Open-i returned non-list 'list' field (type={type(items).__name__}); "
