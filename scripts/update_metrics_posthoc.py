        ...
        for task_dir in approach_dir.iterdir():
            ...
            for model_dir in task_dir.iterdir():
                ...
                for timestamp_dir in timestamp_dirs:
                    ...
                    individual_dirs = [
                        d
                        for d in timestamp_dir.iterdir()
...
def backup_metrics_file(metrics_file: Path) -> Path:
    ...
    backup_file = (
        metrics_file.parent / f"{metrics_file.stem}_backup_{int(time.time())}{metrics_file.suffix}"
    )
...
def update_consolidated_metrics(result_dir: ResultDirectory, force: bool = False) -> bool:
    ...
    if existing_metrics_file and not force:
        logger.info(
            f"Metrics already exist at {existing_metrics_file}, skipping (use --force to override)"
        )
        return False

    try:
        logger.info(
            f"Re-evaluating consolidated metrics for {result_dir.approach}/{result_dir.task}/{result_dir.model}"
        )
...
def update_individual_metrics(result_dir: ResultDirectory, force: bool = False) -> tuple[int, int]:
    ...
    logger.info(
        f"Re-evaluating {total_count} individual samples for {result_dir.approach}/{result_dir.task}/{result_dir.model}"
    )
...
def aggregate_individual_metrics(result_dir: ResultDirectory) -> dict | None:
...
Examples:
  ...
  python scripts/update_metrics_posthoc.py --results-dir runs/full_benchmark --task diagnosis --model "google_gemini-2.5-flash-preview-05-20"
    ...
    parser.add_argument(
        "--results-dir", type=str, required=True, help="Base directory containing benchmark results"
    )
    parser.add_argument(
        "--approach",
        type=str,
    ...
    parser.add_argument(
        "--task", type=str, help="Filter by task (localization, caption, diagnosis)"
    )
    parser.add_argument("--model", type=str, help="Filter by model name")
    parser.add_argument(
        "--force", action="store_true", help="Force update even if metrics already exist"
    )
    parser.add_argument(
        "--aggregate-only",
        action="store_true",
    ...
    result_directories = discover_result_directories(
        results_dir, approach_filter=args.approach, task_filter=args.task, model_filter=args.model
    )
    ...
    # Summary
    logger.info(f"\n{'=' * 60}")
    logger.info("Post-hoc metrics update completed!")
    ...
    if total_success > 0:
        ...
        logger.info("  python scripts/generate_latex_tables.py")
        logger.info("  python scripts/generate_figures.py")
