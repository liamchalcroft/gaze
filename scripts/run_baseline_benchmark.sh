...
OUTPUT_DIR="$PWD/runs/full_benchmark/baseline"
BATCH_SIZE=1
MAX_ITERS=-1
...
echo "==== BASELINE BENCHMARK (Enhanced System Prompts) ===="
echo "DATA_DIR   = ${DATA_DIR}"
echo "OUTPUT_DIR = ${OUTPUT_DIR}"
echo "MODELS     = ${MODELS_DEFAULT[*]}"
echo "TASKS      = ${TASKS[*]}"
echo "APPROACH   = baseline (optimized with enhanced JSON format)"
echo "=========================================================="

for MODEL in "${MODELS_DEFAULT[@]}"; do
  for TASK in "${TASKS[@]}"; do
    RUN_DIR="${OUTPUT_DIR}/${TASK}/$(echo "${MODEL}" | tr '/:' '_')"
    echo -e "\n▶ baseline (enhanced) | ${TASK} | ${MODEL}\n   → ${RUN_DIR}"
    
    # Create run directory and log configuration
    mkdir -p "${RUN_DIR}"
    echo "Configuration: approach=baseline, task=${TASK}, model=${MODEL}" > "${RUN_DIR}/config.txt"
    echo "Enhanced features: JSON format specification, clinical accuracy emphasis" >> "${RUN_DIR}/config.txt"
...
echo "\n✅ Baseline benchmark finished → ${OUTPUT_DIR}" 