...
def load_individual_metrics(metrics_file: Path) -> Optional[Dict[str, float]]:
...
def aggregate_task_metrics(task: str, metrics_list: List[Dict[str, float]]) -> Dict[str, float]:
    ...
    if task == "localization":
        ...
        return {
            "iou": np.mean([m.get("detection_mAP50", 0.0) for m in metrics_list]),  # Use mAP50 as IoU proxy
