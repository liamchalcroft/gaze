...
def get_system_info():
    ...
    print(f"   Total memory: {memory_gb:.1f} GB" if isinstance(memory_gb, float) else f"   Total memory: {memory_gb}")
    print(f"   Available memory: {available_gb:.1f} GB" if isinstance(available_gb, float) else f"   Available memory: {available_gb}")
    print()
...
def run_benchmark(args):
    ...
    if args.benchmark == "full":
        ...
        cmd = [
            ...
            "--output-dir", args.output_dir,
            "--batch-size", str(args.batch_size),
            "--max-parallel-models", str(args.max_parallel_models)
        ]
    else:
...
def main():
    ...
    # Info command
    info_parser = subparsers.add_parser("info", help="Show system info and recommendations")
    
    # Run command
    run_parser = subparsers.add_parser("run", help="Run parallel benchmarks")
    run_parser.add_argument("benchmark", choices=["full", "comprehensive"], 
                           help="Which benchmark to run")
    run_parser.add_argument("--data-dir", default="~/data/nova", 
                           help="Path to NOVA dataset")
    run_parser.add_argument("--output-dir", default="runs/parallel_benchmark",
                           help="Output directory")
    run_parser.add_argument("--batch-size", type=int, default=4,
                           help="Batch size for processing")
    run_parser.add_argument("--max-parallel", type=int, default=6,
                           help="Max parallel approaches (for full benchmark)")
    run_parser.add_argument("--max-parallel-models", type=int, default=3,
                           help="Max parallel models (for single approach)")
    run_parser.add_argument("--max-iters", type=int, default=-1,
                           help="Max iterations (-1 for all)")
