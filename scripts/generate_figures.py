...
def create_performance_comparison_bar_chart(df: pd.DataFrame, output_file: Path) -> None:
    ...
    metrics_data = {"Localization mAP50": [], "Caption BLEU": [], "Diagnosis Top-1": []}
    ...
    for i, (metric, values) in enumerate(metrics_data.items()):
        ax = axes[i]
        bars = ax.bar(
            approach_labels, values, color=colors[i], alpha=0.8, edgecolor="black", linewidth=0.5
        )
...
def create_task_specific_comparison(df: pd.DataFrame, output_dir: Path) -> None:
    ...
    for task in tasks:
        ...
        available_approaches = [
            app for app in all_approaches if app in task_data["approach"].values
        ]
...
def create_sample_count_visualization(df: pd.DataFrame, output_file: Path) -> None:
    ...
    approach_stats = (
        df.groupby("approach").agg({"sample_count": "sum", "failed_samples": "sum"}).reset_index()
    )

    approach_stats["success_rate"] = (
        (approach_stats["sample_count"] - approach_stats["failed_samples"])
        / approach_stats["sample_count"]
        * 100
    )
    ...
    all_approaches = ["baseline", "multiturn", "web_search", "visual", "comprehensive"]
    available_approaches = [
        app for app in all_approaches if app in approach_stats["approach"].values
    ]

    # Reorder approach_stats to match our standard order
    approach_stats = (
        approach_stats.set_index("approach").reindex(available_approaches).reset_index()
    )
    ...
    # Add value labels
    for bar, value in zip(bars1, approach_stats["sample_count"], strict=False):
        height = bar.get_height()
        ax1.text(
            bar.get_x() + bar.get_width() / 2.0,
            height + 0.5,
    ...
    # Add value labels
    for bar, value in zip(bars2, approach_stats["success_rate"], strict=False):
        height = bar.get_height()
        ax2.text(
            bar.get_x() + bar.get_width() / 2.0,
            height + 0.5,
...
def create_heatmap(df: pd.DataFrame, output_file: Path) -> None:
    ...
    ax.set_title("Performance Heatmap Across Tasks and Approaches", fontweight="bold", pad=20)
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default="outputs/figures", help="Directory to save figures"
    )
    parser.add_argument(
        "--figure-type",
    ...
    parser.add_argument(
        "--format", choices=["png", "pdf", "svg"], default="png", help="Output format for figures"
    )
    ...
    if args.figure_type in ["all", "samples"]:
        create_sample_count_visualization(df, output_dir / f"sample_counts.{args.format}")
