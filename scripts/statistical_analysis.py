...
def extract_metric_values(sample_data: Dict[str, Dict[str, List[Dict[str, float]]]], 
                         approach: str, task: str, metric: str) -> List[float]:
...
def perform_paired_ttest(values_a: List[float], values_b: List[float], 
                        approach_a: str, approach_b: str, task: str, metric: str) -> Optional[StatisticalTest]:
...
def perform_wilcoxon_test(values_a: List[float], values_b: List[float],
                         approach_a: str, approach_b: str, task: str, metric: str) -> Optional[StatisticalTest]:
...
def perform_mcnemar_test(values_a: List[float], values_b: List[float],
                        approach_a: str, approach_b: str, task: str, metric: str) -> Optional[StatisticalTest]:
...
def perform_all_statistical_tests(sample_data: Dict[str, Dict[str, List[Dict[str, float]]]]) -> List[StatisticalTest]:
    ...
    logger.info(f"Performing statistical tests for {len(approaches)} approaches and {len(tasks)} tasks")
        ...
        for approach_a, approach_b in itertools.combinations(task_approaches, 2):
            ...
            for metric in metrics:
                ...
                if is_binary_metric(metric):
                    ...
                    test_result = perform_mcnemar_test(values_a, values_b, approach_a, approach_b, task, metric)
                    if test_result:
                        results.append(test_result)
                else:
                    # Both parametric and non-parametric tests for continuous metrics
                    ttest_result = perform_paired_ttest(values_a, values_b, approach_a, approach_b, task, metric)
                    if ttest_result:
                        results.append(ttest_result)
                    
                    wilcoxon_result = perform_wilcoxon_test(values_a, values_b, approach_a, approach_b, task, metric)
                    if wilcoxon_result:
                        results.append(wilcoxon_result)
    
    return results

def apply_multiple_testing_correction(results: List[StatisticalTest], method: str = "bonferroni") -> List[StatisticalTest]:
...
def save_statistical_results(results: List[StatisticalTest], output_dir: Path) -> None:
    ...
    significant_results = df[df['significant'] == True]
...
def generate_statistical_report(results: List[StatisticalTest], output_dir: Path) -> None:
    ...
    for task, task_results in by_task.items():
        ...
        for result in task_results:
            ...
            report_lines.append(f"- {comparison} on {metric_name}: p = {result.p_value:.4f}{effect_desc}")
...
def main():
    parser = argparse.ArgumentParser(description="Perform statistical significance testing on benchmark results")
    parser.add_argument(
        "--results-dir",
        type=str,
    ...
    logger.info(f"Statistical analysis completed: {significant_count}/{len(results)} tests significant")
